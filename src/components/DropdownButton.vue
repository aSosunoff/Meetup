<template>
	...
	>
		...
		>
			...
			/>
			<template v-if="Boolean(value)"> {{ title }} - {{ selectedElement.text }} </template>
			<template v-else>
				{{ title }}
			</template>
		</button>
		...
		>
			<button
				...
				:class="{
					dropdown__item_icon: isIcon
				}"
				type="button"
			>
				<app-icon :icon="item.icon" v-if="'icon' in item" />
				{{ item.text }}
			</button>
		</div>
	</div>
